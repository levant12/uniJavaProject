...
public class ScaledController implements Initializable {
    ...
    private String calculatePoints(double geoP, double foreignP, String chosenSub, double chosenSubP) {
        ...
        savedResult.setGrant(calculateGrant(geoP, foreignP, chosenSubP, df));

        String resultText = "Georgian scaled point: " + df.format(geoP) + "\n"
                + "Foreign Language scaled point: " + df.format(foreignP) +"\n"
                + chosenSub + " scaled point: " + df.format(chosenSubP) + "\n"
    ...
    private int calculateGrant(double geoScaledP, double foreignScaledP, double chosenSubScaledP, DecimalFormat df) {
        ...
        savedResult.setCalculated(Double.valueOf(df.format(res)));
    ...
    public void onBaseClick(ActionEvent event) throws IOException {
